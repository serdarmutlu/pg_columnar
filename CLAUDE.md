...
## Phase 4 — Metadata Backup/Reconstruction

`columnar_rebuild_metadata(regclass)` is a SQL-callable disaster-recovery function
that reconstructs the metadata file by scanning the stripe directory.

### Use case

The metadata file is the only file that columnar cannot recover from the Arrow IPC
data alone.  If it is lost or corrupted (hardware failure, accidental deletion,
filesystem corruption), a `SELECT` on the table returns 0 rows and subsequent INSERTs
silently overwrite existing stripe IDs.  `columnar_rebuild_metadata` recovers the
table from the `.arrow` files alone.

### Algorithm

1. **Flush write buffer** (if any) so in-progress rows become a normal on-disk stripe.
2. **Acquire advisory write lock** (`columnar_acquire_write_lock`) to prevent concurrent
   writes during the reconstruction.
3. **Scan the stripe directory** for `stripe_XXXXXX.arrow` files using `opendir` /
   `readdir`.  The filename check uses an explicit `.arrow` suffix test:
   ```c
   if (namelen < 7 || strcmp(de->d_name + namelen - 6, ".arrow") != 0) continue;
   ```
   This is critical: `sscanf(name, "stripe_%d.arrow", &sid)` alone is NOT sufficient —
   it returns 1 for `stripe_000001.deleted` because `%d` matches `000001` and sscanf
   stops on the first format mismatch without failing the scan.
4. **Sort stripe IDs** numerically with `qsort`.
5. **For each stripe file:**
   - `stat()` to get `file_size`.
   - Read entire file into a `palloc`'d buffer.
   - **Detect compression** from the first 4 bytes:
     - `0x28 0xB5 0x2F 0xFD` → ZSTD
     - `0x04 0x22 0x4D 0x18` → LZ4
     - Anything else → NONE (raw Arrow IPC)
   - **Get uncompressed size:**
     - ZSTD: `ZSTD_getFrameContentSize(data, file_size)` (always set; written by `ZSTD_compress`)
     - LZ4: `LZ4F_getFrameInfo(dctx, &fi, data, &consumed)` → `fi.contentSize`
       (always set; written with `prefs.frameInfo.contentSize = ...` in `columnar_write_stripe`)
     - NONE: `uncompressed_size = file_size`
   - **Decompress** into a nanoarrow `ArrowBuffer` (ZSTD/LZ4) or copy raw bytes (NONE).
   - **Read Arrow IPC** via `ArrowIpcInputStreamInitBuffer` → `ArrowIpcArrayStreamReaderInit`
     → `get_schema` → `get_next` → `batch.length` = row count.
   - **Count deleted rows** via `columnar_read_delete_bitmap` → popcount.
6. **Write reconstructed metadata** via `columnar_write_metadata` (which also updates the
   metadata cache).
7. **Return** `(stripes_rebuilt int, rows_total bigint)` where `rows_total` is the total
   live row count (= Σ(row_count_i − deleted_i) across all recovered stripes).

### Error handling

Stripe files that cannot be read (stat failure, fopen failure, decompression error,
IPC parse error) are **skipped with a WARNING**; the rest are still recovered.

### Usage

```sql
-- Recover table with a lost or corrupt metadata file
SELECT * FROM columnar_rebuild_metadata('orders_columnar');
-- Returns: (stripes_rebuilt, rows_total)

-- Safe to run on a healthy table (metadata equivalent to original)
SELECT * FROM columnar_rebuild_metadata('orders_columnar');
```

### What is NOT reconstructed

The `.stats` files (per-stripe min/max statistics) are not re-generated by the rebuild —
they remain on disk unchanged.  The stripe pruning for new scans works as before.
Similarly, `.deleted` bitmaps are read but not altered.

### sscanf suffix-match bug (fixed during development)

The initial implementation used `sscanf(de->d_name, "stripe_%d.arrow", &sid)` to detect
stripe files.  This has a subtle bug: `sscanf` returns the count of successfully-assigned
conversion specifiers.  For `"stripe_000001.deleted"`, `%d` successfully matches `000001`
and sscanf then encounters a format/input mismatch (`.a` vs `.d`) but has already
